    ...
    for i in xrange(record_num):
        for j in xrange(record_num):
            ...
            if density_array[i] > max_density:
                ...
                max_density_index = i
    return density_array,max_density

def compute_density_distance(density_array, max_density, max_distance, distance_array):
    record_num = len(density_array)
    distance_result = np.zeros(record_num)
    for i in xrange(record_num):
        if  density_array[i] == max_density:
            tmp_max_distance = 0
            for value in distance_array[i]:
                if value > tmp_max_distance:
                    tmp_max_distance = value
            distance_result[i] = tmp_max_distance
        else:
